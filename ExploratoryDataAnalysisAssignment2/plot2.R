...
# Have total emissions from PM2.5 decreased in the Baltimore City from 1999 to 
...
plot2  <- function()  {
    ...
    pm25.by.year <- nei %>% filter(fips=="24510") %>% group_by(year) %>% summarize(total.pm25 = sum(Emissions)) %>% arrange
    
    filename<- "./plot2.png"
    png(filename=filename, width=600, height=600, units="px")
    plotPM25(pm25.by.year, "Baltimore City")
    dev.off()
